...
이 저장소의 내용을 Organization의 `.github` 저장소에 복사합니다.

```
Organization 이름/.github/   ← 이 저장소
├── .github/workflows/
│   ├── pr-review.yml
│   └── issue-response.yml
├── scripts/
...
### 2단계: GitHub Actions Secrets 설정

Organization 또는 저장소의 `Settings → Secrets and variables → Actions`에서 설정합니다.
...
### 3단계: Organization Actions 권한 허용

`Organization Settings → Actions → General`에서 다음을 확인합니다:
- "Allow all actions and reusable workflows" 또는 이 저장소의 워크플로우 허용

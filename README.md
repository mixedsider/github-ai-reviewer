...
> Organization 전체에 적용하려면 `Organization → Settings → Secrets and variables → Actions`에서 Organization Secret으로 등록하면 됩니다.
...
jobs:
  respond:
    uses: mixedsider/github-ai-reviewer/.github/workflows/issue-response.yml@main
    with:
      repository: ${{ github.repository }}

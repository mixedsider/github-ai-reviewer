...
5. 주요 발견사항은 최대 5개로 제한하고, 각 항목은 반드시 "[심각]", "[위험]", "[주의]", "[일반]" 중 하나로 시작하세요.
...
9. 주요 발견사항은 심각 > 위험 > 주의 > 일반 순서로 정렬하세요.

주요 발견사항 등급 기준:
- [심각]: 보안 취약점, 데이터 손상, 장애, 빌드/런타임 실패처럼 배포를 막아야 하는 문제
- [위험]: 특정 조건에서 명확한 버그나 운영 문제가 발생할 수 있는 문제
- [주의]: 지금 당장 실패하지는 않지만 유지보수/테스트/성능 리스크가 있는 문제
- [일반]: 가벼운 개선 제안. 확실한 근거가 없으면 작성하지 마세요.

응답은 반드시 JSON 형식으로만 하세요:
{
  "overall_assessment": "변경 범위와 전반적 위험도를 짧게 평가",
  "findings": "주요 발견사항. 최대 5개, 각 항목은 [심각]/[위험]/[주의]/[일반]로 시작. 없으면 없음",
...
def _split_finding_items(text: str) -> list[str]:
    items = []
    current = []
...
def _finding_level(item: str) -> tuple[str, str]:
    level_patterns = [
        ("심각", r"^\s*(?:\*\*)?\[?(?:심각|critical|blocker|p0|p1)\]?(?:\*\*)?\s*[:：-]?\s*"),
        ("위험", r"^\s*(?:\*\*)?\[?(?:위험|high|major|p2)\]?(?:\*\*)?\s*[:：-]?\s*"),
        ("주의", r"^\s*(?:\*\*)?\[?(?:주의|medium|minor|p3)\]?(?:\*\*)?\s*[:：-]?\s*"),
        ("일반", r"^\s*(?:\*\*)?\[?(?:일반|low|info|nit|p4)\]?(?:\*\*)?\s*[:：-]?\s*"),
    ]
    for level, pattern in level_patterns:
        if re.match(pattern, item, flags=re.IGNORECASE):
            return level, re.sub(pattern, "", item, count=1, flags=re.IGNORECASE).strip()
    return "일반", item.strip()


def prepare_findings_section(value, empty_text: str = "없음") -> str:
    text = normalize_review_text(value, empty_text)
    if text == empty_text:
        return empty_text

    findings = []
    for item in _split_finding_items(text):
        level, body = _finding_level(item)
        body = re.sub(r"^주요 발견사항\s*[:：-]?\s*", "", body).strip()
        if body and body.lower() not in EMPTY_SECTION_VALUES:
            findings.append((FINDING_LEVELS.index(level), level, truncate_review_text(body, 350)))
    ...
    return "\n".join(
        f"- [{level}] {body}"
        for _, level, body in findings[:MAX_FINDINGS]
    )
...
def handle_pr():
    ...
    comment = loader.render("pr_review.md", {
        ...
        "findings": prepare_findings_section(ai_result.get("findings"), "없음"),
